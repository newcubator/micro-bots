...
module.exports.bot = async event => {
  ...
  const birthdays = users.filter(user => !!user.birthday && user.birthday.endsWith(searchDate));
  birthdays.forEach(async (birthday) => {
    console.log(birthday);
  
    const channelName = `cupcake2`;
    const channelResponse = await slack.conversations.create({name: channelName, is_private: false})
    ...
    const invites = userResponse.members
      ...
      .join(',');
    const inviteresponse = await slack.conversations.invite({channel: channelResponse.channel.id, users: invites})
