...
const createDayRange = (from: Dayjs, to: Dayjs) =>
  new Array(to.diff(from, "day") + 1).fill(from).map((date, idx) => date.add(idx, "day"));
...
const extractExpectedHours = (employment: MocoEmployment, day: Dayjs) =>
  employment.pattern.am[day.get("day") - 1] + employment.pattern.pm[day.get("day") - 1];
...
export function calculateWorkload(
  ...
  return createDayRange(dayjs(from), dayjs(to))
    ...
    .map((day: Dayjs) => {
      ...
      const employment: MocoEmployment = employments.find(
        ({ from, to }: MocoEmployment) =>
    ...
    .reduce(
      (
        ...
        dayObject: DayObjectType & {
          holidays: number;
          vacations: number;
          notPlannedHours: number;
          sick: number;
          absence: number;
        }
      ) => {
        ...
        return {
          ...
          days: [
            ...workload.days,
            { day: dayObject.day, worked: dayObject.worked, expectedHours: dayObject.expectedHours },
          ],
