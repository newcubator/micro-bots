...
function createMailSignature(user: MocoUserType): string {
  ...
  const cta = `<span style="display: block; text-align: left"><a target="_blank" rel="noopener noreferrer" ${
    user.custom_properties.CTA_Link ? `href = "${user.custom_properties.CTA_Link}"` : ""
  } color="#6A78D1" style="border-width: 6px 12px; border-style: solid; border-color: #405780; display: inline-block; background-color: #405780; color: rgb(255, 255, 255); font-weight: 700; text-decoration: none; text-align: center; line-height: 40px; font-size: 12px; border-radius: 3px;">${
    user.custom_properties.CTA_Text
  }</a></span>`;
  ...
  }, Amtsgericht Hannover HRB 221930</span> </span> </td></tr></tbody> </table> ${
    user.custom_properties.CTA_Text ? cta : ""
