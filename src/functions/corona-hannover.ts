...
const VACCINATION_CENTER_HANNOVER =
  "https://www.impfportal-niedersachsen.de/portal/rest/appointments/findVaccinationCenterListFree/30159?stiko=&count=1&birthdate=825202800000";
  ...
  return new Promise<boolean>(async (resolve, reject) => {
    if (!GOOGLE_CREDENTIALS) {
      getSecret().then(() => {
        ...
        return sheetsAccessor.setupGoogle(GOOGLE_CREDENTIALS).then(async () => {
...
const request = async (sheetsAccessor: SheetsAccessor) => {
  return await axios
    .get(VACCINATION_CENTER_HANNOVER, {})
    .then(async (response) => {
      const res: ResponseType = response.data;
      if (res.succeeded) {
        await check(response.data, sheetsAccessor);
      } else {
        console.log("something went wrong.", JSON.stringify(response.data));
      }
    })
...
const check = async (result: ResponseType, sheetsAccessor: SheetsAccessor) => {
  ...
  promises.push(sheetsAccessor.getRows().then((value) => (isNotified = value)));

  return await Promise.all(promises).then(() => {
    result.resultList.forEach(async (value) => {
      console.log(`Out of Stock: ${value.outOfStock}\t freeSlotSizeOnline: ${value.freeSlotSizeOnline}`);
