...
it("handle interaction for book support without creating index", async () => {
  eventBridgeSendMock.mockResolvedValueOnce({});

  const result = await interactionHandler(samplePayload9);
...
it("handle interaction for book support with creating index", async () => {
  eventBridgeSendMock.mockResolvedValueOnce({});

  const result = await interactionHandler(samplePayload10);

  expect(eventBridgeSendMock).toHaveBeenCalledWith({
    text: "CREATE Testfrage",
    responseUrl: "https://hooks.slack.com/services/T00000000/B00000000/XXXXXXXXXXXXXXXXXXXXXXXX",
    channelId: "C02BBA8DWVD",
    actionType: ActionType.BOOK_SUPPORT,
    createIndex: true,
