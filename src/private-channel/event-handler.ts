import { PrivateChannelRequestedEvent, UnLockProjectRequestedEvent } from '../slack/interaction-handler';
...
export const eventHandler = async (event: EventBridgeEvent<string, PrivateChannelRequestedEvent>) => {
    ...
    const excludedUsersNames = excludedUsers.map(async (user) => await getRealSlackName(user))
